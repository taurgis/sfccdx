...
describe('attributegroup-get', () => {
    ...
    describe('All Attribute Groups of all objects', () => {
        it('should save all Attribute Groups for all objects', async () => {
            await attributeGroupGet({})

            let resultGroupWithoutDefinitions = {...dummyAttributeGroup}
            delete resultGroupWithoutDefinitions.attribute_definitions;

            expect(writeFileSpy.callCount).to.equal(8);

            // First Object
            // First group
            expect(writeFileSpy.firstCall.args[0]).to.contain(path.join('object1', 'groups', 'group1.json'));
            expect(writeFileSpy.firstCall.args[1]).to.equal(JSON.stringify(cleanOCAPIResponse(resultGroupWithoutDefinitions), null, 4));
            expect(writeFileSpy.secondCall.args[0]).to.contain(path.join('object1', 'groups', 'group1-assignments.json'));
            expect(writeFileSpy.secondCall.args[1]).to.equal(JSON.stringify(['attribute1', 'attribute2'], null, 4));

            // Second group
            expect(writeFileSpy.thirdCall.args[0]).to.contain(path.join('object1', 'groups', 'group2.json'));
            expect(writeFileSpy.thirdCall.args[1]).to.equal(JSON.stringify(cleanOCAPIResponse(resultGroupWithoutDefinitions), null, 4));
            expect(writeFileSpy.getCall(3).args[0]).to.contain(path.join('object1', 'groups', 'group2-assignments.json'));
            expect(writeFileSpy.getCall(3).args[1]).to.equal(JSON.stringify(['attribute1', 'attribute2'], null, 4));

            // Second Object
            // First group
            expect(writeFileSpy.getCall(4).args[0]).to.contain(path.join('object2', 'groups', 'group1.json'));
            expect(writeFileSpy.getCall(4).args[1]).to.equal(JSON.stringify(cleanOCAPIResponse(resultGroupWithoutDefinitions), null, 4));
            expect(writeFileSpy.getCall(5).args[0]).to.contain(path.join('object2', 'groups', 'group1-assignments.json'));
            expect(writeFileSpy.getCall(5).args[1]).to.equal(JSON.stringify(['attribute1', 'attribute2'], null, 4));

            // Second group
            expect(writeFileSpy.getCall(6).args[0]).to.contain(path.join('object2', 'groups', 'group2.json'));
            expect(writeFileSpy.getCall(6).args[1]).to.equal(JSON.stringify(cleanOCAPIResponse(resultGroupWithoutDefinitions), null, 4));
            expect(writeFileSpy.getCall(7).args[0]).to.contain(path.join('object2', 'groups', 'group2-assignments.json'));
            expect(writeFileSpy.getCall(7).args[1]).to.equal(JSON.stringify(['attribute1', 'attribute2'], null, 4));
            ...
            // Should call the OCAPI
            expect(getAttributeGroupForObjectStub.callCount).to.equal(8);
            expect(getAttributeGroupsForObjectStub.calledTwice).to.be.true;
            expect(getObjectTypesOCAPICallStub.calledOnce).to.be.true;
        });

        it('should output an error if we get an error response from the OCAPI when fetching all objects', async () => {
            sinon.restore();
            ...
            // Should call the OCAPI
            expect(getAttributeGroupsForObjectStub.calledOnce).to.be.false;
            expect(getAttributeGroupForObjectStub.called).to.be.false;
            expect(getObjectTypesOCAPICallStub.called).to.be.true;
        });
        ...
        it('should output an error if we get an error response from the OCAPI when fetching an Attribute Group', async () => {
            ...
            expect(outputFieldsSpy.called).to.be.false;
            expect(outputSuccessSpy.called).to.be.false;
            expect(outputErrorSpy.callCount).to.equal(4);
            expect(outputErrorSpy.firstCall.args[0]).to.contain('Test Error');
            expect(outputErrorSpy.secondCall.args[0]).to.contain('Test Error');
            expect(outputErrorSpy.thirdCall.args[0]).to.contain('Test Error');
            expect(outputErrorSpy.getCall(3).args[0]).to.contain('Test Error');
        ...
        it('should output debug information if the debug flag is set', async () => {
            ...
            // Output
            expect(outputFieldsSpy.callCount).to.equal(4);
            expect(outputSuccessSpy.callCount).to.equal(4);
            expect(outputCommandBookEndSpy.calledTwice).to.be.true;
        });
