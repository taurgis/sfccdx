...
const { authenticate, SystemObjectDefinition } = require('../api/OCAPI');
...
function saveToFile(object, attribute, content) {
    const basePath = `${process.cwd()}${path.sep}data${path.sep}meta${path.sep}system-objecttype-extensions${path.sep}${object}${path.sep}`;

    if (!fs.existsSync(basePath)) {
        fs.mkdirSync(basePath, { recursive: true });
    }

    fs.writeFileSync(`${basePath + attribute}.json`, JSON.stringify(cleanOCAPIResponse(content), null, 4));
}
...
async function getObjectsIDs(systemObjectDefinition) {
    ...
    if (result.isSuccess()) {
        resultingIDs = result.data.data.filter((object) => {
...
async function getAttributeIDsFromObject(systemObjectDefinition, object, includeSystemAttributes) {
    ...
    if (result.isSuccess()) {
        if (result.data.data) {
            resultingIDs = result.data.data.filter((attribute) => {
...
module.exports = async function attributeGet(options) {
    try {
        ...
        const accessToken = await authenticate(options.clientid, options.clientsecret);
        const systemObjectDefinition = new SystemObjectDefinition({ 'hostname': options.hostname, 'token': accessToken });
